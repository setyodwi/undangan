...
const API_URL = "GANTI_DENGAN_URL_APPS_SCRIPT";

/* Load gallery dari Google Sheet */
async function loadGalleryFromSheet() {
  const container = document.getElementById("masonry");
  if (!container || !API_URL) return;

  try {
    const response = await fetch(API_URL + "?action=gallery");
    ...
    console.error("Gallery gagal dimuat:", err);
  }
}


/* Load ucapan dari Google Sheet */
async function loadUcapanFromSheet() {
    ...
    data.forEach((item) => {
      container.innerHTML += `
        <div class="u-item">
          <div class="u-body">
            <div class="u-head">
              <span class="u-name">${escapeHTML(item.nama)}</span>
              <span class="u-badge">${item.hadir || ""}</span>
            </div>
            <div class="u-msg">${escapeHTML(item.pesan)}</div>
          </div>
        </div>
      `;
    });

    const count = document.getElementById("ucapan-count");
    if (count) count.textContent = "(" + data.length + ")";

  } catch (err) {
    console.error("Ucapan gagal dimuat:", err);
  }
}
...
async function sendToSheet(data) {
  ...
  try {
    ...
    console.error("Gagal kirim sheet:", err);
  }
}
